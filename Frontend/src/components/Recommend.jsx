...
//           <h2 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 animate-gradient">
  ...
  return (
    <section id="recommend" className="py-16 bg-gradient-to-r from-gray-900 via-black to-gray-900 text-white">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredDestinations.map((destination, index) => (
            <TiltCard key={index} destination={destination} handleBooking={handleBooking} index={index} />
          ))}
        </div>
      </div>
    </section>
...
// TiltCard Component
const TiltCard = ({ destination, handleBooking, index }) => {
  const cardRef = useRef(null);
  const animationFrameRef = useRef(null);

  // Determine if the card is in an even or odd row
  ...
  const animate = () => {
    ...
    const rotateX = Math.sin(time) * 5; // Oscillate between -5 and 5 degrees
    const rotateY = (Math.cos(time) * 5) * (isOddRow ? -1 : 1); // Invert direction for odd rows
